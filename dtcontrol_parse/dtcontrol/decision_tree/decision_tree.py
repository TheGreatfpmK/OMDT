...
class Node:
    ...
    @staticmethod
    def from_json_dict(json_dict, splitting_strategies, impurity_measure, depth=0):
        node = Node(splitting_strategies, impurity_measure, depth=depth)
        if json_dict["actual_label"] is not None:
            node.actual_label = json_dict["actual_label"]
            node.index_label = json_dict["actual_label"]  # Assuming index_label is same as actual_label for simplicity
        if json_dict["split"] is not None:
            split_class = next((s for s in splitting_strategies if s.__class__.__name__ == json_dict["split"]["class"]), None)
            if split_class is None:
                raise ValueError(f"Unknown split class: {json_dict['split']['class']}")
            node.split = split_class.from_json_dict(json_dict["split"])
            for child_dict in json_dict["children"]:
                child_node = Node.from_json_dict(child_dict, splitting_strategies, impurity_measure, depth + 1)
